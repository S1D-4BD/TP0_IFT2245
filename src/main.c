...
typedef int error_code; //so this here ca vx dire que les int vont avoir un sens, qqch de positif = reussite, sinon echec
...
int strcmp(const char *p1, const char *p2) {
    char c1, c2;
    do {
        c1 = (char) *p1++;          //this means qu'on prend le char pointé par le pointeur p1
        c2 = (char) *p2++;          //mm chose
        if (c1 == '\0')             //here on fait une vrai comparaison de fin de mot si le char 1 est  = 0
            return c1 - c2;         //on return la difference des chars (du ascii)
    } while (c1 == c2);             //et on fait tt ca tant que ya pas de difference (donc quils ont les mm suite de chars depuis le debut)
    return c1 - c2;
}
